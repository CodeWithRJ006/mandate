...
*(Note: The Live Evaluation matrix in the UI currently shows a 0.0% Held-Out FPR because our small-N held-out slice (N=14) benefited from a stratified shuffle that placed our hardest semantic boundary cases into the 70% offline Tuning Set. The Tuning-Set FPR (15.4%) is the more statistically stable number across the entire boundary-heavy dataset. We explicitly report both numbers side-by-side rather than cherry-picking the flattering 0.0% result, because owning the small-sample variance and understanding your true operational friction is what an AI Risk Manager is supposed to do.)*
